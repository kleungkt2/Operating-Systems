...
//    printf("==> sys_open, BEGIN\n");
    ...
    if (omode & O_CREATE) {
        ...
        if ((ip = namei(path)) == 0) {
//            printf("==> sys_open, ip = namei(path) == 0 \n");
            end_op();
            return -1;
        }
        ilock(ip);
...
//            printf("==> sys_open, ip->type == T_SYMLINK\n");
            if ((omode & O_NOFOLLOW) == 0) {
                for (int i = 0; i < 10; ++i) {
                    if ((n = readi(ip, 0, (uint64) target, 0, MAXPATH)) <= 0) {
//                        printf("==> sys_open, O_FOLLOW, readi() error\n");
                        iunlockput(ip);
                        end_op();
                        return -1;
                    }

//                    printf("==> sys_open, path = %s, target = %s\n", path, target);
